...
document.addEventListener("keyup", function(event) {
    ...
    if (isEventInTextInput) handleTextInput(event.target.value); // event 
});
document.addEventListener("click", function(event) {
    ...
    else if (isEventOnCheckbox) handleCheckbox(event);
});
...
function handleCheckbox(event) {
    ...
    const isAllUnchecked = [
        ...checkboxInputs
    ].every((checkboxInput)=>checkboxInput.checked === false);
    console.log("isAllUnchecked", isAllUnchecked);
    if (isAllUnchecked) resetButtonState();
    else resetButtonStateWhenEdit();
}
